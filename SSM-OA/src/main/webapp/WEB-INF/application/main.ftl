		...
		var zNodes =[
			<#if menuList??>
				<#list  menuList  as menu>
					{ id:1, pId:0, name:<#if menu.menuName??>'${menu.menuName}'</#if>, url:<#if menu.menuPath??>'${c.contextPath}/${menu.menuPath}'</#if>, target:"right"},
				</#list>
	</#if>
...
<#if menuList??>
	...
	<#if menuList?exists>
		<#list menuList as menu>
			<tr>
			<td><#if menu.menuName??>'${menu.menuName}'</#if>--<#if menu.menuPath??>'${menu.menuPath}'</#if><br></td>
			</tr>
		</#list>
	</#if>
<br>
